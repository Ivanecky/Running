...
## FUNCTION TO SCRAPE INDIVIDUAL RUNNERS PAGE
```{r}
...
runners = runners %>%
  mutate(
    AVG_TIME_FORM = paste0(floor(AVG_TIME/60), ":", round((AVG_TIME%%60), 2))
  ) %>%
  mutate(
    PR_FORM = paste0(floor(PR/60), ":", round((PR%%60), 2))
  ) %>%
  filter(!(EVENT %in% c("event", "OTHER")))
```

### REFORMAT TO ONE ROW PER RUNNER
```{r}
runners = runners %>%
  group_by(NAME) %>%
  summarise(
...
```{r}
# Replace all the Inf for numeric columns only
temp = runners %>%
